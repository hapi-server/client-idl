...
function cmb_date,epochsin,cdate=cdate,format=format0,todms = todms,time_in_julday = time_in_julday, idiag= idiag, fout = fout, summary=summary
...
if KEYWORD_SET(summary) then begin
   epr = cmb_minmax(epochs)
   if cmb_var_type(epr) eq 'LONG64' then cf = 1d9 else cf = 1d3
   dates = cmb_date( epr ,format=format0,todms = todms,time_in_julday = time_in_julday )
   print,'time range: ', dates, ' DT=', (epr[1]-epr[0])/cf
   fnyq = nyquist_frequency(epochs)
   return, 1
endif

if cmb_var_type(epochs) ne 'DOUBLE' then return, fix(epochs)
if n_elements(format0) ne 0 then format=format0 else format = 'isodate' ;'yyyy-mmdd-hhmm-ss.sss'
if KEYWORD_SET(fout) then format= 'yyyy-mmdd-hhmm-ss.sss'
...
for ik =0l,n-1 do begin
    ...
    if  cmb_var_type(epochsin[ik]) eq 'LONG64' then cdf_epoch, epochsin[ik],yr,month,dom,hr,minu,sec,msec,/break
    ...
    yroff = 1900
    if yr[0] ge 2000 then yroff = 2000
    cdate={yr:yr,doy:doy,month:month,dom:dom,hr:hr,min:minu,sec:sec,msec:msec, yroff:yroff}
    ;if keyword_set( idiag ) then print,'cdate:', cdate & help, yr,month,dom,hr,minu,sec,msec, tod_fracday
    ;if KEYWORD_SET( idiag ) then stop
case (format) of
 'spice':begin
 ...
 'isodate' or 'zulu' or 'yyyy-mm-ddThh:mm:ss.sssZ': $
  ...
  'zulu2' or 'yyyy-mm-ddThh:mm:ssZ': $
           date = string( format= '(i4.4,"-", i2.2,"-",i2.2,"T",i2.2,":",i2.2,":",i2.2,"Z")' , yr, month, dom, hr,minu,sec)
...
'isodate3' or 'yyyy-mm-ddThh:mm': $
      date = string( format= '(i4.4,"-", i2.2,"-",i2.2,"T",i2.2,":",i2.2)' , yr, month, dom, hr,minu)
 'a': $
...
'MMM dd, yyyy': $
 ...
 'cdaweb':date = string(yr, month, dom, hr,minu,sec,msec $
                               ,format= '(i4.4,"/",i2.2,"/",i2.2,x,i2.2,":",i2.2,":",i2.2,".",i3.3)')
...
'yyyymmddhhmm':date = string( yr,month,dom,hr,minu, $
 ...
 'yyyymmddhhmmss':date = string( yr,month,dom,hr,minu,sec, $
               format= '(i4,5(i2.2))')
'yyyy-mmdd-hhmm-ss':date = string( yr,month,dom,hr,minu,sec, $
               format= '(i4,"-",2(i2.2),"-",2(i2.2),"-",i2.2)')
'yyyy-mmdd-hhmm-ss.sss':date = string( yr,month,dom,hr,minu,sec, msec, $
               format= '(i4,"-",2(i2.2),"-",2(i2.2),"-",i2.2,".",i3.3)')
 'yyyymmddhhmmss.sss':date = string( yr,month,dom,hr,minu,sec,msec, $
               format= '(i4,5(i2.2),".",i3.3)')
 'yyyy-mm-dd-hh-mm-ss.sss':date = string( yr,month,dom,hr,minu,sec,msec, $
               format= '(i4,5("-",i2.2),".",i3.3)')
 'yyyy-mm-dd-hh-mm':date = string( yr,month,dom,hr,minu, $
               format= '(i4,4("-",i2.2))')
 'yyyy,mm,dd,hh,mm,ss.sss':date = string( yr,month,dom,hr,minu,sec,msec, $
