...
info = CREATE_STRUCT('hapiversion',j.hapi, 'id', id)
fields =['startdate','stopdate', 'sampleStartDate','sampleStopDate','description', 'resourceURL', 'resourceID',  'creationDate','modificationDate','cadence','format','contact','contactID']
for i=0,n_elements(fields)-1 do $
  if cmb_tag_name_exists(fields[i], j,ip) then info = CREATE_STRUCT(info, fields[i], j.(ip))

recepoch = {var_type:'TT2000',units:'ms'}
